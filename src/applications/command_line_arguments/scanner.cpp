...
namespace emu::applications::command_line_arguments {
    ...
    void Scanner::read_token(const std::string &line) {
        m_current_pos = 0;
    ...
    bool Scanner::handle_short_option(const std::string &line) {
        if (line[m_current_pos] == '-') {
            const bool is_next_hyphen = m_current_pos < line.length() - 1 && line[m_current_pos + 1] == '-';
            if (is_next_hyphen) {
                return false;
            }
    ...
    bool Scanner::handle_long_option(const std::string &line) {
        if (line[m_current_pos] == '-') {
            const bool is_next_hyphen = m_current_pos < line.length() - 1 && line[m_current_pos + 1] == '-';
            const bool is_next_next_alnum = m_current_pos < line.length() - 2 && std::isalnum(line[m_current_pos + 2]);
    ...
    bool Scanner::handle_identifier(const std::string &line) {
