...
void TilemapPane::draw(char const* title, const u32 tile_texture, bool* p_open)
{
    ...
    if (!m_is_debug_container_set) {
        ...
        ImGui::Text("The tilemap is not provided to this pane."); // NOLINT(*-pro-type-vararg)
    } else {
...
{
    const std::vector<std::shared_ptr<Tile>> tiles = m_debug_container->tiles().at(palette_idx);
    const std::size_t number_of_tiles = tiles.size();
    const std::size_t tile_size = tiles.at(0)->size();
    ...
    for (unsigned int row = 0; row < rows; ++row) {
        for (unsigned int col = 0; col < tiles_per_row; ++col) {
            ...
            tile->map_to_framebuffer(m_framebuffers.at(palette_idx), row * tile_size, col * tile_size);
        }
    }
...
void TilemapPane::render_image(const u32 tile_texture) const
{
    glBindTexture(GL_TEXTURE_2D, tile_texture);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, //
        m_framebuffers.at(m_chosen_palette_idx).to_output_vector().data());
