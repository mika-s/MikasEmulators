...
namespace emu::z80 {
     ...
     * @param cycles is the number of cycles variable, which will be mutated
     */
    void sra_MixyPd(u16 ixy_reg, u8 d, const EmulatorMemory &memory, Flags &flag_reg,
     ...
     * @param cycles is the number of cycles variable, which will be mutated
     */
    void sra_MixyPd_r(u8 &reg, u16 ixy_reg, u8 d, const EmulatorMemory &memory, Flags &flag_reg,
                      unsigned long &cycles
    ) {
        u8 value = memory[ixy_reg + static_cast<i8>(d)];
        sra(value, flag_reg);
